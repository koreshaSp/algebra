...
\begin{example}
    ...
    мы знали посещаемость каждого сайта в нашей сети руководствуясь
    следующими правилами. Пусть $w_i$~--- рейтинг посещаемости $i$ узла сети,
    после посещения этого узла пользователь с вероятностью $d_i^{out}$
    переходит на страницу $out$. Можно считать, что сумма исходящих
    вероятностей для каждой вершины равно $1$, так как в противном случае
    можем добавить ребро в саму же вершину с необходимым весом.
    Хотим назначить рейтинг популярности
    так, чтобы выполнялось следующее равенство для всех $i$.
    $$w_i = \sum_{j\rightarrow i} \frac{w_j}{d_j^{i}}$$

    Для решения задачи составим систему для всех $i$ и запишем
    её в виде матриц. 
    А именно запишем систему 
    в виде: $w = P\times w$, где $w$~--- искомый вектор, а $P$~--- матрица заданная
    следующим образом:
    \[
        P_{i,j} = \begin{cases}
            0, \text{если нет ребра } \{i,j\},\\
            d^i_{j}, \text{если есть ребро } \{i,j\}
        \end{cases}
    \] 
    ...
    системы есть нетривиальное решение? У нас есть
    ...
    Значит $\dim \ker(P - E_n) \ge 1$, следовательно $\exists w\not=0\colon Pw = w$,
    Что и требовалось доказать.

    В действительности очень часто ещё и единственный (то есть $\ker(P-E_n)=1$).
    Но в общей ситуации это конечно же не верно, например, когда $P = E_n$.
\end{example}
...
\end{motivation}
\begin{definition}
    Вернетреугольной матрицей назовём матрицу вида:
    \[
        \begin{pNiceMatrix}
            * & \Hdotsfor{2} & *\\
            0 & \Ddots & & \Vdots\\
    ...
    Нижнетреугольной назовём матрицу вида: 
\[
        \begin{pNiceMatrix}
            * & 0 & \Cdots & 0\\
            \Vdots & \Ddots & \Ddots & \Vdots\\
...
\begin{definition}
    ...
    \[
        ...
        &    1 & & \Vdots & & & \Block{2-2}<\LARGE>{0}\\
        &     & \Ddots[line-style=standard] \\
        & & & \blue 1    \\
        & & & \Vdots & \Ddots[line-style=standard] \\
\blue i \rightarrow & \Cdots & & \blue \lambda & \Cdots & \blue 1 & \Cdots\\
        & \Block{2-2}<\LARGE>{0} & & \Vdots & & &\Ddots[line-style=standard]  \\
        & & & \Vdots & & & & 1  \\
        & & & \blue \overset{\uparrow}{j}
        \end{pNiceMatrix}
